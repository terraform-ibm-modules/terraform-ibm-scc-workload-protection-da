...
| <a name="input_existing_scc_cos_kms_key_crn"></a> [existing\_scc\_cos\_kms\_key\_crn](#input\_existing\_scc\_cos\_kms\_key\_crn) | (OPTIONAL) The CRN of an existing KMS key to be used to encrypt the SCC COS bucket. If no value is passed, a value must be passed for either the existing\_kms\_instance\_crn input variable if you want to create a new key ring and key, or the existing\_scc\_cos\_bucket\_name input variable if you want to use an existing bucket. | `string` | `null` | no |
